...
public class DocumentConfigurations : IEntityTypeConfiguration<Document>
{
    ...
    public void Configure(EntityTypeBuilder<Document> builder)
    {
        ...
        builder.HasKey(e => e.DocId).HasName("PK_Documentos_v2");
        ...
        builder.Property(e => e.CodigoDeBarras).IsUnicode(false);
        builder.Property(e => e.FechaBaja).HasColumnType("datetime");
        builder.Property(e => e.FechaCreacion).HasColumnType("datetime");
        builder.Property(e => e.FechaPago)
            .HasColumnType("datetime")
            .HasColumnName("fechaPago");
        builder.Property(e => e.IdDetalleDePago).HasColumnName("idDetalleDePago");
        builder.Property(e => e.IdDocument).HasColumnName("idDocument");
        builder.Property(e => e.Moneda)
            .HasMaxLength(4)
            .IsUnicode(false);
        builder.Property(e => e.MontoBruto).HasColumnType("money");
        ...
        builder.Property(e => e.UserCreate)
            .HasMaxLength(100)
            .IsUnicode(false);
        builder.Property(e => e.VencimientoCaecai).HasColumnName("VencimientoCAECAI");

        builder.HasOne(d => d.State).WithMany(d => d.Documents)
            .HasForeignKey(d => d.EstadoId)
            .HasConstraintName("FK__Documento__Estad__267ABA7A");

        builder.HasOne(d => d.PaymentDetail).WithMany(p => p.Documents)
            .HasForeignKey(d => d.IdDetalleDePago)
            .HasConstraintName("FK_Documentos_DetalleDePago");

        builder.HasOne(d => d.Currency).WithMany(p => p.Documents)
            .HasForeignKey(d => d.Moneda)
            .HasConstraintName("FK__Documento__Moned__276EDEB3");

        builder.HasOne(d => d.Provider).WithMany(p => p.Documents)
            .HasForeignKey(d => d.ProveedorCuit)
            .HasConstraintName("FK_Documentos_Proveedores");

        builder.HasOne(d => d.Society).WithMany(p => p.Documents)
            .HasForeignKey(d => d.SociedadCuit)
            .HasConstraintName("FK__Documento__Socie__2A164134");

        builder.HasOne(d => d.DocumentType).WithMany(p => p.Documents)
